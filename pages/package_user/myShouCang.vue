<template>
	...
	<view class="container flex-box flex-column">
		<!-- 可拖动tab -->
		<scroll-view id="tab-bar" class="scroll-h bd-bottom" :scroll-x="true" :show-scrollbar="false" :scroll-into-view="scrollInto"
			...
			<swiper-item class="swiper-item" v-for="(tab,index1) in allData" :key="index1">
				...
				<scroll-view class="scroll-v list" enableBackToTop="true" scroll-y="true" style="height: 100%;" @scrolltolower="loadMore(index1)">
					...
					<view v-if="index1==0" v-for="(item,index) in allData[0]" :key="index">
						...
						<text-new v-if="item.news_type==1" :newsData="item"></text-new>
						<new-list v-if="item.news_type==2" :newsData="item" :textOver2="true" :isReverse="true"></new-list>
						<!-- 大封面 -->
						<big-cover v-if="item.news_type==3" :hasBottom="true" :coverData="item"></big-cover>
						<!-- 三张图片 -->
						<three-new v-if="item.news_type==4" :threeData="item"></three-new>
					</view>
					
					<view v-if="index1==1" v-for="(item,index) in allData[1]" :key="index">
