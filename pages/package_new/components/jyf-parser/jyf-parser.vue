<template>
	<view>
		...
		<view id="top" :style="showAm+(selectable?';user-select:text;-webkit-user-select:text':'')" class="all">
			<!--#ifdef H5-->
			<div :id="'rtf'+uid"></div>
			<!--#endif-->
			<!--#ifndef H5-->
	...
	export default {
		...
		methods: {
			...
			setContent(html, append) {
				...
				for (let i = 0, j = 0, img; img = imgs[i]; i++) {
					...
					if (!img.hasAttribute('ignore') && img.parentElement.nodeName != 'A') {
						...
						img.onclick = function() {
							var preview = true;
							that.$commonJs.previewImage(this.i, _ts.imgList, img.i);
				...
				// h5视频处理
				// #ifdef H5
				var iframes = this.rtf.getElementsByTagName('iframe');
				// console.log(iframes)
				...
				// 获取播放纯文字
				// var spans = this.rtf.getElementsByTagName('span');
				// console.log(spans)
				// for (i = 0; i < spans.length; i++) {
				// 	var texts = spans[i].innerText 
				// 	console.log(texts)
					
				// 	uni.setStorage({
				// 	key:'articles',
				// 	data:JSON.stringify(spans[i].innerText)
				// 	})
				// }
